...
int city_Block_distance(struct Image data,struct Image data_te,int x,int y,int minD){
    int D = 0;
    for(int m = 0;m <= data_te.height-1;m++){
        for(int n = 0;n <= data_te.width-1;n++){
            D += abs(data.image[y+m][x+n]-data_te.image[m][n]);
            if(D > minD){
                return INT_MAX;
            }
        }
...
struct Image template_matching(struct Image data,struct Image data_te){
    int minD = INT_MAX;
    int op_x = -1;
    int op_y = -1;
    for(int y = 0;y <= data.height - data_te.height;y++){ 
        for(int x = 0;x <= data.width - data_te.width;x++){
            int D = city_Block_distance(data,data_te,x,y,minD);
            if(D < minD){
                minD = D;
                op_x = x;
                op_y = y;
...
int main(int argc, char *argv[]) {
    ...
    Image_data = template_matching(Image_data, Image_data_te);
